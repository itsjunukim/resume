...
#resume-item[
  ...
  #resume-subsection("Achievement")[
    - *[SKB 지능망 통합]* 이기종 DB 간 데이터 정합성 검증 프로세스 설계 및 SIPP 시뮬레이션을 통해 1,000만 가입자 DB 무중단 마이그레이션 성공
    - *[삼성SDS 레거시 현대화]* 노후 서버를 최신 고성능 장비로 전면 교체하고 OS 최적화를 수행하여 인프라 처리 성능 향상
    - *[안정성 확보]* 120개 이상의 부하 테스트 시나리오를 수행하여 Cut-over 후 장애 발생 0건 기록
  ]
]